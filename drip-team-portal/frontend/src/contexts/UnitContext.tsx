...
export const UnitProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateUnitSetting = (dimension: string, setting: UnitSetting) => {
  ...
  const getDimensionFromUnit = (unit: string): string | null => {
    return UNIT_TO_DIMENSION[unit] || null;
  };

  const getUserUnit = (dimension: string): string => {
    return unitSettings[dimension]?.scale || DEFAULT_UNIT_SETTINGS[dimension]?.scale || '';
  };

  const convertToUserUnit = (value: number, fromUnit: string, dimension: string): number => {
